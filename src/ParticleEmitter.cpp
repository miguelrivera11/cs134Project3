...
void ParticleEmitter::spawn(float time) {
	...
	case DiscEmitter:   // x-z plane
	  {  
		...
		float speed = velocity.length();
		particle.position = position + (dir.getNormalized() * radius);
		//ofVec3f XZposition = dir.getNormalized() * radius;
		//particle.position = ofVec3f(XZposition.x + position.x, position.y, XZposition.z + position.z);
